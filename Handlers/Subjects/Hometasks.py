...
@Hometasks_Router.callback_query(F.data == 'edit', Subject_actions.switch_choose)
...
@Hometasks_Router.message(F.text, Subject_actions.edit_SubjectName)
...
@Hometasks_Router.message(F.text, Subject_actions.edit_SubjectGroup)
...
@Hometasks_Router.callback_query(F.data == 'text', Subject_actions.switch_type)
...
@Hometasks_Router.callback_query(F.data.in_(['photo','data']), Subject_actions.switch_type)
...
@Hometasks_Router.message(F.text, Subject_actions.edit_SubjectContent_Count)
...
@Hometasks_Router.message(F.photo or F.document, Subject_actions.edit_SubjectContent)
...
@Hometasks_Router.message(F.text, Subject_actions.edit_SubjectContent)
...
@Hometasks_Router.callback_query(F.data == 'no', Subject_actions.switch_comments)
async def hometask_edit_over(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    async with async_session() as session:
        us_repo = UserRepository(session)
        user_service = UserService(repo=us_repo)
        sub_repo = SubjectRepository(session)
        subject_service = SubjectService(repo=sub_repo)
...
@Hometasks_Router.callback_query(F.text, Subject_actions.edit_SubjectComment)
async def subject_edit_comment(msg: types.Message, state: FSMContext):
    data = await state.get_data()
    async with async_session() as session:
        us_repo = UserRepository(session)
        user_service = UserService(repo=us_repo)
